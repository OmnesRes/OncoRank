...
##f=open(os.path.join(BASE_DIR,'onco_lnc','mrna','cox','BLCA','coeffs_pvalues.txt'))
##BLCA=sorted([[j.split('\t')[0],float(j.split('\t')[-2])] for j in f],key=lambda x:x[-1])
##
##f=open(os.path.join(BASE_DIR,'onco_lnc','mrna','cox','BRCA','coeffs_pvalues.txt'))
##BRCA=sorted([[j.split('\t')[0],float(j.split('\t')[-2])] for j in f],key=lambda x:x[-1])
##                           
##f=open(os.path.join(BASE_DIR,'onco_lnc','mrna','cox','CESC','coeffs_pvalues.txt'))
##CESC=sorted([[j.split('\t')[0],float(j.split('\t')[-2])] for j in f],key=lambda x:x[-1])
##
##f=open(os.path.join(BASE_DIR,'onco_lnc','mrna','cox','COAD','coeffs_pvalues.txt'))
##COAD=sorted([[j.split('\t')[0],float(j.split('\t')[-2])] for j in f],key=lambda x:x[-1])
##
##f=open(os.path.join(BASE_DIR,'onco_lnc','mrna','cox','ESCA','coeffs_pvalues.txt'))
##ESCA=sorted([[j.split('\t')[0],float(j.split('\t')[-2])] for j in f],key=lambda x:x[-1])
##
##f=open(os.path.join(BASE_DIR,'onco_lnc','mrna','cox','GBM','coeffs_pvalues.txt'))
##GBM=sorted([[j.split('\t')[0],float(j.split('\t')[-2])] for j in f],key=lambda x:x[-1])
##
##f=open(os.path.join(BASE_DIR,'onco_lnc','mrna','cox','HNSC','coeffs_pvalues.txt'))
##HNSC=sorted([[j.split('\t')[0],float(j.split('\t')[-2])] for j in f],key=lambda x:x[-1])
##
##f=open(os.path.join(BASE_DIR,'onco_lnc','mrna','cox','KIRC','coeffs_pvalues.txt'))
##KIRC=sorted([[j.split('\t')[0],float(j.split('\t')[-2])] for j in f],key=lambda x:x[-1])
##                           
##f=open(os.path.join(BASE_DIR,'onco_lnc','mrna','cox','KIRP','coeffs_pvalues.txt'))
##KIRP=sorted([[j.split('\t')[0],float(j.split('\t')[-2])] for j in f],key=lambda x:x[-1])
##
##f=open(os.path.join(BASE_DIR,'onco_lnc','mrna','cox','LAML','coeffs_pvalues.txt'))
##LAML=sorted([[j.split('\t')[0],float(j.split('\t')[-2])] for j in f],key=lambda x:x[-1])
##
##f=open(os.path.join(BASE_DIR,'onco_lnc','mrna','cox','LGG','coeffs_pvalues.txt'))
##LGG=sorted([[j.split('\t')[0],float(j.split('\t')[-2])] for j in f],key=lambda x:x[-1])
##
##f=open(os.path.join(BASE_DIR,'onco_lnc','mrna','cox','LIHC','coeffs_pvalues.txt'))
##LIHC=sorted([[j.split('\t')[0],float(j.split('\t')[-2])] for j in f],key=lambda x:x[-1])
##
##f=open(os.path.join(BASE_DIR,'onco_lnc','mrna','cox','LUAD','coeffs_pvalues.txt'))
##LUAD=sorted([[j.split('\t')[0],float(j.split('\t')[-2])] for j in f],key=lambda x:x[-1])
##
##f=open(os.path.join(BASE_DIR,'onco_lnc','mrna','cox','LUSC','coeffs_pvalues.txt'))
##LUSC=sorted([[j.split('\t')[0],float(j.split('\t')[-2])] for j in f],key=lambda x:x[-1])
##
##f=open(os.path.join(BASE_DIR,'onco_lnc','mrna','cox','OV','coeffs_pvalues.txt'))
##OV=sorted([[j.split('\t')[0],float(j.split('\t')[-2])] for j in f],key=lambda x:x[-1])
##
##f=open(os.path.join(BASE_DIR,'onco_lnc','mrna','cox','PAAD','coeffs_pvalues.txt'))
##PAAD=sorted([[j.split('\t')[0],float(j.split('\t')[-2])] for j in f],key=lambda x:x[-1])
##
##f=open(os.path.join(BASE_DIR,'onco_lnc','mrna','cox','READ','coeffs_pvalues.txt'))
##READ=sorted([[j.split('\t')[0],float(j.split('\t')[-2])] for j in f],key=lambda x:x[-1])
##
##f=open(os.path.join(BASE_DIR,'onco_lnc','mrna','cox','SARC','coeffs_pvalues.txt'))
##SARC=sorted([[j.split('\t')[0],float(j.split('\t')[-2])] for j in f],key=lambda x:x[-1])
##
##f=open(os.path.join(BASE_DIR,'onco_lnc','mrna','cox','SKCM','coeffs_pvalues.txt'))
##SKCM=sorted([[j.split('\t')[0],float(j.split('\t')[-2])] for j in f],key=lambda x:x[-1])
##
##f=open(os.path.join(BASE_DIR,'onco_lnc','mrna','cox','STAD','coeffs_pvalues.txt'))
##STAD=sorted([[j.split('\t')[0],float(j.split('\t')[-2])] for j in f],key=lambda x:x[-1])
##
##f=open(os.path.join(BASE_DIR,'onco_lnc','mrna','cox','UCEC','coeffs_pvalues.txt'))
##UCEC=sorted([[j.split('\t')[0],float(j.split('\t')[-2])] for j in f],key=lambda x:x[-1])
##
###determine genes in the population and make dictionaries
##
##all_cancers=[BLCA,BRCA,CESC,COAD,ESCA,GBM,HNSC,KIRC,KIRP,LAML,LGG,LIHC,LUAD,LUSC,OV,\
...
####    values.append(sum(temp)/float(count))
##    values.append(temp)
##f=open('mrna_values.txt','w')
##f.write(str(values))
##f.close()
...
f=open('mrna_values.txt')
values=eval(f.read())
values=[list(np.array(i)/930.0*100) for i in values]
